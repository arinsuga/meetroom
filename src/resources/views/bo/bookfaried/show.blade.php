...
@section('content_title', 'Booking Faried Info')
...
@if ($viewModel->data->orderstatus_id == 3)
    <!-- button Reopen -->
    <!-- <li class="nav-item">
        <a class="nav-link" href="{{ route('bookfaried.reopen', ['bookfaried' => $viewModel->data->id]) }}">
...
<!-- button edit -->
<li class="nav-item">
    <a class="nav-link" href="{{ route('bookfaried.edit', ['bookfaried' => $viewModel->data->id]) }}">
...
<li class="nav-item">
    ...
    <form id="frmData" role="form" id="frmData" method="POST" action="{{ route('bookfaried.destroy', ['bookfaried' => $viewModel->data->id]) }}">
        @csrf
        @method('DELETE')
    </form>
</li>
